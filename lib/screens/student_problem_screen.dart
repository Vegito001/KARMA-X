...
class StudentProblemScreen extends StatefulWidget {
  ...
  const StudentProblemScreen(
      {super.key, required this.playerName, required this.schedule});
...
class _StudentProblemScreenState extends State<StudentProblemScreen>
  ...
  // Which backend actually produced the last result, and whether Modal
  // failed and Gemini was used instead. Populated from AiService after
  // every call so you can see this on screen instead of guessing.
  String? _source;
  ...
  Future<void> _analyse() async {
    ...
    if (text.length < 20) {
      setState(() =>
          _errorMsg = 'Please describe your problem in at least a few words.');
      return;
    }
    _focusNode.unfocus();
    setState(() {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _analysedProblem = problem;
          _isAnalysing = false;
          _source =
              AiService.lastSource; // 'modal' | 'gemini' | 'gemini_fallback'
          _fallbackWarning =
              AiService.lastError; // non-null only if Modal failed
        });
      }
    } catch (e) {
      if (mounted) {
        setState(() {
          _isAnalysing = false;
          _errorMsg = 'Analysis failed. Check your connection.\n$e';
          _source = AiService.lastSource;
          _fallbackWarning = AiService.lastError;
        });
      }
    }
  }
  ...
  Widget _sourceBadge() {
    ...
    final isModal = _source == 'modal';
    final isFallback = _source == 'gemini_fallback';
    final label = isModal
        ? 'MODAL (KARMAX)'
        : isFallback
            ? 'GEMINI (FALLBACK)'
    ...
    return Padding(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('ANSWERED BY: $label',
            style: AppTheme.monoFont(size: 10, color: color, letterSpacing: 1)),
      ]),
    );
  }

  Widget _fallbackWarningBanner() {
    if (_fallbackWarning == null) return const SizedBox.shrink();
    return Padding(
      padding: const EdgeInsets.only(bottom: 12),
      child: Container(
        padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(
            color: AppTheme.bg800, border: Border.all(color: AppTheme.danger)),
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Icon(Icons.warning_amber_rounded,
              color: AppTheme.danger, size: 14),
          const SizedBox(width: 8),
          Expanded(
              child: Text(_fallbackWarning!,
                  style: AppTheme.monoFont(size: 11, color: AppTheme.danger))),
        ]),
      ),
    );
  }
              ...
              child: Padding(
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      Text(
                          'Type your main struggle as a student in your own words.\nKarmaX AI will analyse it and build your quest path.',
                          style: AppTheme.monoFont(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: TextField(
                          ...
                          minLines: 4,
                          style: AppTheme.monoFont(
                              size: 13, color: AppTheme.text100),
                          cursorColor: AppTheme.mana,
                          decoration: InputDecoration(
                            hintText:
                                'e.g. "I keep procrastinating on assignments until the last night..."',
                            hintStyle: AppTheme.monoFont(
                                size: 12, color: AppTheme.text600),
            ...
            Padding(
              ...
              child: Column(children: [
                if (_analysedProblem == null && !_isAnalysing)
                  GestureDetector(
                    onTap:
                        _inputCtrl.text.trim().length >= 20 ? _analyse : null,
                    child: AnimatedContainer(
                      duration: const Duration(milliseconds: 250),
                      width: double.infinity,
                      padding: const EdgeInsets.symmetric(vertical: 18),
                      decoration: BoxDecoration(
                        color: _inputCtrl.text.trim().length >= 20
                            ? AppTheme.mana
                            : AppTheme.bg700,
                        border: Border.all(
                            color: _inputCtrl.text.trim().length >= 20
                ...
                if (_analysedProblem != null && !_isAnalysing) ...[
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Center(
                          child: Text('INSPECT ROOT CAUSES  >',
                              style: AppTheme.displayFont(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ..._loadLines.asMap().entries.map((e) => AnimatedOpacity(
              ...
              child: Padding(
                  ...
                  child: Text(e.value,
                      style:
                          AppTheme.monoFont(size: 11, color: AppTheme.mana))),
            )),
      ]),
    );
  }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Expanded(
              child: Column(
                ...
                Text(p.title.toUpperCase(),
                    style:
                        AppTheme.displayFont(size: 14, color: AppTheme.mana)),
                if (p.subtitle.isNotEmpty) ...[
                  const SizedBox(height: 3),
                  Text(p.subtitle,
                      style:
                          AppTheme.monoFont(size: 11, color: AppTheme.text400))
                ],
              ])),
          Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
              decoration: BoxDecoration(
                  border: Border.all(color: AppTheme.mana),
                  color: AppTheme.mana.withValues(alpha: 0.08)),
              child: Text('AI SCAN',
        ...
        if (p.summary.isNotEmpty) ...[
          ...
          Text('> AI ANALYSIS',
              style: AppTheme.monoFont(
                  size: 9, color: AppTheme.text400, letterSpacing: 2)),
          const SizedBox(height: 6),
          Text(p.summary,
          ...
          ...p.causes.map((c) => Padding(
              ...
              child:
                ...
                Expanded(
                    child: Text(c,
                        style: AppTheme.monoFont(
                            size: 12, color: AppTheme.text100))),
              ]))),
        ],
      ]),
    );
        ...
        child: SafeArea(
          child: Column(children: [
                ...
                child: Column(
                    ...
                    children: [
                      FadeTransition(
                        ...
                        child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 8),
                              Text(
                                  'Be specific — the more detail you give, the more personalised your quests will be.',
                                  style: AppTheme.monoFont(
                        ...
                        child: Column(
                            ...
                            children: [
                              ...
                              TextField(
                                ...
                                decoration: InputDecoration(
                                  hintText:
                                      'e.g. "I keep procrastinating on assignments until the last night and then panic. I also can\'t focus because I keep checking my phone..."',
                                  hintStyle: AppTheme.monoFont(
                                      size: 12, color: AppTheme.text600),
                                  contentPadding:
                          ...
                          children: [
                            ...
                            'Can\'t focus',
                              ...
                              .map((e) => GestureDetector(
                                    onTap: () {
                                      _inputCtrl.text =
                                          'I struggle with $e as a student. ';
                                      _inputCtrl.selection =
                                          TextSelection.fromPosition(
                                              TextPosition(
                                                  offset:
                                                      _inputCtrl.text.length));
                                      setState(() {});
                                    },
                                    child: Container(
                                      ...
                                      decoration: BoxDecoration(
                                        ...
                                        border:
                                            Border.all(color: AppTheme.bg500),
                      ...
                      if (_analysedProblem != null && !_isAnalysing) ...[
                        ...
                        _sourceBadge(),
                        _fallbackWarningBanner(),
                        _gestaltResultCard(_analysedProblem!),
                      ],
            ...
            Container(
              ...
              child: Column(children: [
                if (_analysedProblem == null && !_isAnalysing)
                  _gestaltPrimaryButton(
                      label: '⚡  ANALYSE WITH AI',
                      enabled: hasEnough,
                      onTap: _analyse),
                if (_analysedProblem != null && !_isAnalysing) ...[
                  _gestaltPrimaryButton(
                      label: 'INSPECT ROOT CAUSES  ›',
                      enabled: true,
                      onTap: _proceed),
                  const SizedBox(height: 10),
                  GestureDetector(
  ...
  Widget _gestaltLoadingCard() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
              color: AppTheme.mana.withValues(alpha: 0.08), blurRadius: 20)
        ],
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
        ...
        ..._loadLines.asMap().entries.map((e) => AnimatedOpacity(
              duration: const Duration(milliseconds: 400),
              opacity: _loadStep > e.key ? 1.0 : 0.15,
              child: Padding(
  ...
  Widget _gestaltResultCard(StudentProblem p) {
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
              color: AppTheme.mana.withValues(alpha: 0.15),
              blurRadius: 24,
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: enabled
              ? [
                  BoxShadow(
                      color: AppTheme.mana.withValues(alpha: 0.25),
                      blurRadius: 20,
